...
import { type IActiveNotificationRid, INotification, type IPlayerRid, NotificationServiceFrontend } from "@pc2/api";
...
const SendMessages: FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
  const toast = useToast();

  const player = usePoliticalCapitalSelector((s) => s.playerState.player);
  const fullGameState = usePoliticalCapitalSelector((s) => s.localGameState.fullGameState);

  const [toPlayerRid, setToPlayerRid] = useState<IPlayerRid | undefined>();
      ...
      <div className={styles.playerSelectorContainer}>
        {Object.values(fullGameState.players).map((otherPlayer) => {
          ...
          return (
            ...
            >
              <PlayerName activePlayer={fullGameState.activePlayers[otherPlayer.playerRid]} player={otherPlayer} />
    ...
    <Modal isOpen={isOpen} onClose={onClose}>
      ...
      <ModalContent>
        ...
        <ModalFooter>
          ...
          <Button
            colorScheme="green"
            disabled={toPlayerRid === undefined || message === undefined || fullGameState.gameState.state !== "active"}
            isLoading={isLoading}
            onClick={sendMessage}
          >
            Send message
...
export const Messages: FC<{}> = () => {
  ...
  const playerMessages = allNotifications
    .filter(
      (n) => INotification.isBetweenPlayers(n.notificationDetails) || INotification.isAnonymous(n.notificationDetails),
    )
    .sort((a, b) => (a.createdOn > b.createdOn ? -1 : 1));

  const maybeRenderMessages = (messages: ReactNode[]) => {
          ...
          {maybeRenderMessages(
            playerMessages.map((playerMessage) => {
                ...
                return (
                  <div className={styles.singleMessage} key={playerMessage.activeNotificationRid}>
                    ...
                    <div className={styles.messageDetails}>
                      ...
                      <div className={styles.description}>Sent on {getFakeDate(playerMessage.createdOn)}</div>
                    </div>
                  </div>
                );
              }
              ...
              if (INotification.isBetweenPlayers(playerMessage.notificationDetails)) {
                ...
                const accordingActivePlayer = fullGameState.activePlayers[playerMessage.notificationDetails.fromPlayer];

                return (
                  <div className={styles.singleMessage} key={playerMessage.activeNotificationRid}>
                    <div>
                      <PlayerName activePlayer={accordingActivePlayer} player={accordingPlayer} sizeOverride="lg" />
                    </div>
                    <div className={styles.messageDetails}>
                      <div className={styles.nameAndMessage}>
              ...
              if (INotification.isGame(gameMessage.notificationDetails)) {
                return (
                  <div className={styles.singleMessage} key={gameMessage.activeNotificationRid}>
                    ...
                    <div className={styles.messageDetails}>
                      <div className={styles.nameAndMessage}>{gameMessage.notificationDetails.message}</div>
                      <div className={styles.description}>Sent on {getFakeDate(gameMessage.createdOn)}</div>
                    </div>
                  </div>
                );
              }

              return <div key={gameMessage.activeNotificationRid}>Unknown game message</div>;
            }),
          )}
        </div>
      </div>
      <SendMessages isOpen={isNewMessageOpen} key={isNewMessageOpen ? "open" : "closed"} onClose={closeSendMessages} />
    </div>
  );
};
