...
import { type IActiveStaffer, type IPlayerRid, isRecruit, type IStafferCategory, isTrainer } from "@pc2/api";
...
export const PlayerParty: FC<{ playerRid: IPlayerRid }> = ({ playerRid }) => {
  const [activatingStaffer, setActivatingStaffer] = useState<IActiveStaffer | undefined>();
  ...
  const onBack = () => setActivatingStaffer();
      ...
      <div className={styles.staffersInCategory}>
        {staffersInCategory.map((staffer) => {
          const canActivateCard =
            isViewingSelf && (category === "recruit" || category === "trainer") && staffer.state === "active";
          ...
          return (
            <div className={styles.activateStafferContainer} key={staffer.activeStafferRid}>
              <StafferCard isPlayerStaffer={player?.playerRid === playerRid} staffer={staffer} />
              {canActivateCard && (
                <Button
  ...
  return (
    <div className={styles.overallContainer}>
      <div className={styles.withPlayerName}>
        <PlayerName activePlayer={activePlayer} player={playerDetails} />
