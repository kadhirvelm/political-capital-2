...
export const RecruiterActivation: FC<{
  ...
  const [confirmStaffer, setConfirmStaffer] = useState<IPossibleStaffer | undefined>();

  if (player === undefined || fullGameState === undefined) {
    return null;
  }

  const closeModal = () => setConfirmStaffer();
      ...
      <div className={styles.currentlyRecruiting}>
        ...
        <div className={styles.hiringEvents}>
          {currentlyRecruiting.map((recruitingEvent, index) => (
            <ResolveEvent event={recruitingEvent} key={recruitingEvent.eventDetails.type + index.toString()} />
          ))}
        </div>
      </div>
    );
  ...
  const availableToTrain = Object.values(DEFAULT_STAFFER).filter((staffer) => staffer.upgradedFrom.length === 0);
        ...
        {filteredStaffers.map((staffer) => {
          ...
          return (
            ...
            >
              ...
              <div className={styles.description}>{descriptionOfStaffer(resolvedGameModifiers)[staffer.type]}</div>
